...
var bootstrapPolicy = Ledger{
	...
	Debt: []Baseline{
		...
		},
		{
			...
			ResourceOwner:   "each owning test closes its stream listener and removes duplicate listener-backed coverage; +1/+1 on 2026-08-26 owned by gascity-bh0 (listeningPort in cmd/gc/gc_beads_bd_port_probe_test.go, the one listener site the port-probe tests share to prove a held port)",
	...
	SmallDebt: []Baseline{
		...
		},
		{
			...
			ResourceOwner:   "non-Medium lexical owners move stream-listener tests to exact Medium ownership or replace the listener; +1/+1 on 2026-08-26 owned by gascity-bh0 (listeningPort in cmd/gc/gc_beads_bd_port_probe_test.go, the one listener site the port-probe tests share to prove a held port)",
			MigrationTarget: "P0.4c-listener",
			Expires:         "2026-10-01",
		},
		{

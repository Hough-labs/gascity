...
| --- | --- | --- | --- | --- | --- | --- |
| Audit baseline | all tracked test source | fixed_sleep: 432 calls / 161 files (historical regex census: 447 / 157) | ga-80po0c.2 | tracked test source totals remain visible as audit evidence; ga-80po0c.2 owns this point-in-time source census; +1/+1 on 2026-08-18 owned by gascity-4zf (df0f1ad31, TestCollectLiveWorktreeState_ProtectsDirWithLiveProcess); +1/+1 on 2026-08-26 owned by gascity-t8r1 (the 10ms poll interval of the deadline-bounded zombie-reap loop in cmd/gc/cmd_start_drift_pid_liveness_test.go); +1/+1 on 2026-08-27 owned by gascity-n17v (serializingExecutor.executeCtx in internal/runtime/tmux/new_session_guard_test.go, the file's single sleep: it is the occupancy window TestNewSessionSerializesCreationPerSocket measures, not a delay waiting for a condition. The test fans 12 concurrent NewSession calls at one socket and asserts in-flight creates never exceed 1; with a zero-width critical section an unserialized implementation serializes by accident and the test passes on the bug it exists to catch); +1/+1 on 2026-08-27 owned by gascity-n17v (waitForSession in internal/runtime/tmux/socket_clobber_integration_test.go, integration-tagged so it lands in scope=all only: the poll interval of a deadline-bounded retry loop that returns as soon as the session appears and t.Fatalf's at 10s, the shape already banked for gascity-4zf); -0/-1 on 2026-09-20 owned by gascity-lzzj (internal/api/session_stream_capability_test.go loses its only sleep: TestStreamSessionPeekAcceptsPeekCapability's hand-rolled poll loop over an unsynchronized httptest recorder was the data race the bead exists to fix, and replacing it with the package's shared waitForRecorderSubstring helper moves the poll onto a sleep handler_sessions_test.go already owns. Calls are unchanged; the file drops out of the census) | P0.4a | 2026-10-01 |
...
| Audit baseline | all tracked test source | subprocess: 551 calls / 170 files (historical regex census: 495 / 135) | ga-80po0c.2 | tracked test source totals remain visible as audit evidence; ga-80po0c.2 owns this point-in-time source census; +1/+1 on 2026-08-18 owned by gascity-4zf (df0f1ad31, TestCollectLiveWorktreeState_ProtectsDirWithLiveProcess); +1/+0 on 2026-08-19 owned by gascity-rwu (doctorGitOutput in internal/doctor/checks_test.go, the read-side companion to doctorRunGit); +1/+0 on 2026-08-25 owned by gascity-0zw (runPackScript in examples/bd/dolt/health_test.go, the one exec site every dolt pack-script test now routes through); +1/+1 on 2026-08-26 owned by gascity-t8r1 (startLivenessSubject in cmd/gc/cmd_start_drift_pid_liveness_test.go, the file's single exec site; the four pidGone/waitForPIDExit subjects route through it); +1/+1 on 2026-08-26 owned by gascity-bh0 (runBdScriptHarness in cmd/gc/gc_beads_bd_port_probe_test.go, the one exec site every bd-script port-probe test routes through); +1/+1 on 2026-08-28 owned by gascity-ngab (runGateParallelism in scripts/makefile_gate_parallelism_test.go, the file's single exec site; both the arithmetic table and the fail-closed table route through it. The file covers scripts/test-gate-parallelism, which computes the gate lanes' -parallel bound, and scripts/test-local-concurrency.sh is wired into no make target or CI job, so an in-process shell assertion cannot replace it. Written the obvious way, one exec per table, this would have been +2 calls); -1/+1 on 2026-08-28 owned by gascity-ggq (startProcess in internal/pidutil/pidutil_test.go now carries that file's five spawns on one exec site, which is why calls fall rather than rise; the single added file is internal/nudgepoller/poller_test.go, whose sole site parks a real process carrying the poller argv so TestAliveWithCmdlineRecognizesRealPollerProcess can run the production matcher against a real process-table entry. That composition had no test, which is why AliveWithCmdline's darwin fail-open survived a suite that covered both halves separately); +1/+1 on 2026-09-13 owned by gascity-jiao (runHooksFixtureCommand in scripts/install_git_hooks_test.go, the file's single exec site; git, the installer and the installed hooks all route through it. The file proves scripts/install-git-hooks wires the tracked .githooks up without evicting the tool that owns core.hooksPath, and that is a claim about what git itself runs — real git invoking a real hook is the only thing that can assert it. Written the obvious way, one exec per helper, this would have been +6 calls and +2 files; the doctor half of the same bead reuses initGitRepoOnBranch and adds no site at all) | P0.4a | 2026-10-01 |
...
| Small debt ratchet | all untagged test source | fixed_sleep: 286 calls / 114 files (historical regex census: 287 / 113) | ga-80po0c.2.1 | untagged Small fixed-sleep call/file totals cannot grow; reductions must lower this baseline; non-Medium lexical owners replace elapsed wall time with lifecycle signals; +1/+1 on 2026-08-18 owned by gascity-4zf (df0f1ad31, TestCollectLiveWorktreeState_ProtectsDirWithLiveProcess); +1/+1 on 2026-08-26 owned by gascity-t8r1 (the 10ms poll interval of the deadline-bounded zombie-reap loop in cmd/gc/cmd_start_drift_pid_liveness_test.go); +1/+1 on 2026-08-27 owned by gascity-n17v (serializingExecutor.executeCtx in internal/runtime/tmux/new_session_guard_test.go, the file's single sleep: it is the occupancy window TestNewSessionSerializesCreationPerSocket measures, not a delay waiting for a condition. The test fans 12 concurrent NewSession calls at one socket and asserts in-flight creates never exceed 1; with a zero-width critical section an unserialized implementation serializes by accident and the test passes on the bug it exists to catch); -0/-1 on 2026-09-20 owned by gascity-lzzj (internal/api/session_stream_capability_test.go loses its only sleep: TestStreamSessionPeekAcceptsPeekCapability's hand-rolled poll loop over an unsynchronized httptest recorder was the data race the bead exists to fix, and replacing it with the package's shared waitForRecorderSubstring helper moves the poll onto a sleep handler_sessions_test.go already owns. Calls are unchanged; the file drops out of the census) | W1-W5 | 2026-10-01 |
...
| Small debt ratchet | all untagged test source | net_listen: 96 calls / 37 files (historical regex census: 92 / 34) | ga-80po0c.2.2.2 | untagged Small stream-listener call/file totals cannot grow; reductions must lower this baseline; non-Medium lexical owners move stream-listener tests to exact Medium ownership or replace the listener; +1/+1 on 2026-08-26 owned by gascity-bh0 (listeningPort in cmd/gc/gc_beads_bd_port_probe_test.go, the one listener site the port-probe tests share to prove a held port); +2/+1 on 2026-08-27 owned by gascity-n17v (shortPathSocketFixture and TestNamedSocketHolderReportsLiveListener in internal/runtime/tmux/socket_holder_test.go, the file's only listeners: the code under test tells a live socket holder from a stale socket file, and the refused-with-stable-inode branch is reached only when os.SameFile matches, which recognizes just FileInfo the os package produced — a synthetic fixture cannot reach it. Both listeners are closed by the test that opens them) | P0.4c-listener | 2026-10-01 |
...
| Small debt ratchet | all untagged test source | subprocess: 407 calls / 116 files (historical regex census: 394 / 105) | ga-80po0c.2.1 | untagged Small subprocess call/file totals cannot grow; reductions must lower this baseline; non-Medium lexical owners remove or replace each process call site; +1/+1 on 2026-08-18 owned by gascity-4zf (df0f1ad31, TestCollectLiveWorktreeState_ProtectsDirWithLiveProcess); +1/+0 on 2026-08-19 owned by gascity-rwu (doctorGitOutput in internal/doctor/checks_test.go, the read-side companion to doctorRunGit); +1/+0 on 2026-08-25 owned by gascity-0zw (runPackScript in examples/bd/dolt/health_test.go, the one exec site every dolt pack-script test now routes through); +1/+1 on 2026-08-26 owned by gascity-t8r1 (startLivenessSubject in cmd/gc/cmd_start_drift_pid_liveness_test.go, the file's single exec site; the four pidGone/waitForPIDExit subjects route through it); +1/+1 on 2026-08-26 owned by gascity-bh0 (runBdScriptHarness in cmd/gc/gc_beads_bd_port_probe_test.go, the one exec site every bd-script port-probe test routes through); +1/+1 on 2026-08-28 owned by gascity-ngab (runGateParallelism in scripts/makefile_gate_parallelism_test.go, the file's single exec site; both the arithmetic table and the fail-closed table route through it. The file covers scripts/test-gate-parallelism, which computes the gate lanes' -parallel bound, and scripts/test-local-concurrency.sh is wired into no make target or CI job, so an in-process shell assertion cannot replace it. Written the obvious way, one exec per table, this would have been +2 calls); -1/+1 on 2026-08-28 owned by gascity-ggq (startProcess in internal/pidutil/pidutil_test.go now carries that file's five spawns on one exec site, which is why calls fall rather than rise; the single added file is internal/nudgepoller/poller_test.go, whose sole site parks a real process carrying the poller argv so TestAliveWithCmdlineRecognizesRealPollerProcess can run the production matcher against a real process-table entry. That composition had no test, which is why AliveWithCmdline's darwin fail-open survived a suite that covered both halves separately); +1/+1 on 2026-09-13 owned by gascity-jiao (runHooksFixtureCommand in scripts/install_git_hooks_test.go, the file's single exec site; git, the installer and the installed hooks all route through it. The file proves scripts/install-git-hooks wires the tracked .githooks up without evicting the tool that owns core.hooksPath, and that is a claim about what git itself runs — real git invoking a real hook is the only thing that can assert it. Written the obvious way, one exec per helper, this would have been +6 calls and +2 files; the doctor half of the same bead reuses initGitRepoOnBranch and adds no site at all) | D1/D2/D5/D6/E6 | 2026-10-01 |
...
| Source debt ratchet | all untagged test source | fixed_sleep: 286 calls / 114 files (historical regex census: 295 / 114) | ga-80po0c.2 | untagged fixed-sleep call/file totals cannot grow; reductions must lower this baseline; each owning test replaces elapsed wall time with its lifecycle signal; +1/+1 on 2026-08-18 owned by gascity-4zf (df0f1ad31, TestCollectLiveWorktreeState_ProtectsDirWithLiveProcess); +1/+1 on 2026-08-26 owned by gascity-t8r1 (the 10ms poll interval of the deadline-bounded zombie-reap loop in cmd/gc/cmd_start_drift_pid_liveness_test.go); +1/+1 on 2026-08-27 owned by gascity-n17v (serializingExecutor.executeCtx in internal/runtime/tmux/new_session_guard_test.go, the file's single sleep: it is the occupancy window TestNewSessionSerializesCreationPerSocket measures, not a delay waiting for a condition. The test fans 12 concurrent NewSession calls at one socket and asserts in-flight creates never exceed 1; with a zero-width critical section an unserialized implementation serializes by accident and the test passes on the bug it exists to catch); -0/-1 on 2026-09-20 owned by gascity-lzzj (internal/api/session_stream_capability_test.go loses its only sleep: TestStreamSessionPeekAcceptsPeekCapability's hand-rolled poll loop over an unsynchronized httptest recorder was the data race the bead exists to fix, and replacing it with the package's shared waitForRecorderSubstring helper moves the poll onto a sleep handler_sessions_test.go already owns. Calls are unchanged; the file drops out of the census) | W1-W5 | 2026-10-01 |
...
| Source debt ratchet | all untagged test source | subprocess: 412 calls / 119 files (historical regex census: 380 / 98) | ga-80po0c.2 | untagged subprocess call/file totals cannot grow; reductions must lower this baseline; each process-owning test removes or replaces its source call site; +1/+1 on 2026-08-18 owned by gascity-4zf (df0f1ad31, TestCollectLiveWorktreeState_ProtectsDirWithLiveProcess); +1/+0 on 2026-08-19 owned by gascity-rwu (doctorGitOutput in internal/doctor/checks_test.go, the read-side companion to doctorRunGit); +1/+0 on 2026-08-25 owned by gascity-0zw (runPackScript in examples/bd/dolt/health_test.go, the one exec site every dolt pack-script test now routes through); +1/+1 on 2026-08-26 owned by gascity-t8r1 (startLivenessSubject in cmd/gc/cmd_start_drift_pid_liveness_test.go, the file's single exec site; the four pidGone/waitForPIDExit subjects route through it); +1/+1 on 2026-08-26 owned by gascity-bh0 (runBdScriptHarness in cmd/gc/gc_beads_bd_port_probe_test.go, the one exec site every bd-script port-probe test routes through); +1/+1 on 2026-08-28 owned by gascity-ngab (runGateParallelism in scripts/makefile_gate_parallelism_test.go, the file's single exec site; both the arithmetic table and the fail-closed table route through it. The file covers scripts/test-gate-parallelism, which computes the gate lanes' -parallel bound, and scripts/test-local-concurrency.sh is wired into no make target or CI job, so an in-process shell assertion cannot replace it. Written the obvious way, one exec per table, this would have been +2 calls); -1/+1 on 2026-08-28 owned by gascity-ggq (startProcess in internal/pidutil/pidutil_test.go now carries that file's five spawns on one exec site, which is why calls fall rather than rise; the single added file is internal/nudgepoller/poller_test.go, whose sole site parks a real process carrying the poller argv so TestAliveWithCmdlineRecognizesRealPollerProcess can run the production matcher against a real process-table entry. That composition had no test, which is why AliveWithCmdline's darwin fail-open survived a suite that covered both halves separately); +1/+1 on 2026-09-13 owned by gascity-jiao (runHooksFixtureCommand in scripts/install_git_hooks_test.go, the file's single exec site; git, the installer and the installed hooks all route through it. The file proves scripts/install-git-hooks wires the tracked .githooks up without evicting the tool that owns core.hooksPath, and that is a claim about what git itself runs — real git invoking a real hook is the only thing that can assert it. Written the obvious way, one exec per helper, this would have been +6 calls and +2 files; the doctor half of the same bead reuses initGitRepoOnBranch and adds no site at all) | D1/D2/D5/D6/E6 | 2026-10-01 |
...
through `scripts/test-go-test-shard`, each with its own 15m budget. The other
sharded packages follow in the same loop; see below.
...
`cmd/gc` entirely to the Mac sharded CI matrix job. It does **not** do that for
the other sharded packages — nothing else runs them on Darwin, so it carries
their shards itself.

#### The other oversized packages are sharded the same way
...
`scripts` joined them (gascity-5y4h), from a much lower baseline. Its cost is
not computation: a handful of its tests drive `make` and the real `go` tool
against fixture repos under a scratch `HOME`, so the package's wall time tracks
host contention rather than the code under test. Archived `test-mac` run logs on
one Darwin host show it passing six times between 132s and 636s — a 4.8x spread
across passing runs alone — and hitting the 900s wall on four separate
occurrences. That elasticity, not a fixed cost, is what put it over the
deadline; three of its tests once spent 6m14s of wall clock for 8.3s of user
CPU.

What proves this is a budget problem rather than a broken test is that **a
different set of tests was in flight on each occurrence**. A deterministic
defect implicates the same test every run. One occurrence blocked in `os/exec`
under `TestRuntimeTmuxIntegrationShardPreservesDynamicDiscoveryOnDarwin`;
another had 14 tests that never left `testing.(*T).Parallel` at all, because two
`make`-invoking tests had eaten 9m22s of the budget between them. Since the
package has more than one way to exhaust its budget, capping any single test
family would not converge either.

All three packages are therefore in `$(SHARDED_SWEEP_PKGS)`, excluded from
`$(UNIT_PKGS_SWEEP)`, and run as `SHARDED_UNIT_TOTAL` (default 4) shards
...
bucket actually disagrees with the rest.

Do **not** read a shard number as a fixed assignment of tests. `test-go-test-shard`
distributes round-robin by position in `go test -list` output, so adding a test
shifts every test after it. The shard count buys headroom on the worst bucket,
not a stable partition.

Unlike `cmd/gc`, none of these three is covered by a separate Darwin CI job, so
...
That matters most for `scripts`, which is where the Makefile, push-gate and
CI-policy guards live: dropping it from the Darwin lane the way `cmd/gc` is
dropped would stop the local gate from checking the machinery the gate is built
from.

Three places name this set and must agree: `$(SHARDED_SWEEP_PKGS)`, the
...
genuinely written twice). `TestShardedPackagesAreExcludedFromTheSweep` and
`TestShardedPackagesMatchTheLocalParallelLane` fail the build if they drift —
in either direction, since a package listed in one place but not the other
either runs twice under two deadlines or silently stops being tested.
`TestSweepExclusionMatchesWholeImportPathsOnly` covers the other half: the grep
alternation is anchored on a whole path segment, so `cmd/gc-write-mint` and
`scripts/cipolicy` keep running inside the sweep beside their sharded
namesakes.
...
and split single-package bottlenecks — `cmd/gc`, `examples/gastown`,
`examples/bd/dolt` and `scripts` — across multiple processes.

Use these as the default entry points:

```bash
# Fast unit baseline, with cmd/gc and the other oversized packages sharded.
...
(gascity-cgh) and `$(SHARDED_SWEEP_PKGS)` (gascity-vdhw, gascity-5y4h) — keep the
...
packages anyway — `examples/gastown` has 0 `t.Parallel()` call sites,
`examples/bd/dolt` 1 and `scripts` 17 across 165 top-level tests, and `cmd/gc`
148 across 8285 top-level tests, against 1010 sites in the sweep.
...
| `runtime.builtin.exec` | production_provider | — | `runtime.Provider` | `internal/runtime/t3bridge.NewSeamBacked` | runtime.builtin/prefix:exec: | `runtime.Provider` | waived by gascity-82e through 2026-10-02: the legacy gc-session-t3 prefix branch selects the T3 bridge composition, which has no full shared runtime contract |
...
| `runtime.builtin.herdr` | production_provider | — | `runtime.Provider` | `internal/runtime/herdr.New` | runtime.builtin/exact:herdr | `runtime.Provider` | waived by gascity-82e through 2026-10-02: the existing full conformance run skips in short mode or when the herdr executable is absent |
| `runtime.builtin.hybrid` | production_provider | — | `runtime.Provider` | `cmd/gc.newHybridProvider` | runtime.builtin/exact:hybrid | `runtime.Provider` | waived by gascity-82e through 2026-10-02: cmd/gc.newHybridProvider is the selected registry construction boundary; its internal tmux, K8s, and hybrid constructors are not claimed here, and the wrapper has no full shared runtime contract |
| `runtime.builtin.k8s` | production_provider | — | `runtime.Provider` | `internal/runtime/k8s.NewSeamBacked` | runtime.builtin/exact:k8s | `runtime.Provider` | waived by gascity-82e through 2026-10-02: the actual K8s production composition has no full shared runtime contract |
| `runtime.builtin.ssh` | production_provider | — | `runtime.Provider` | `internal/runtime/ssh.NewSeamBacked` | runtime.builtin/prefix:ssh: | `runtime.Provider` | waived by gascity-82e through 2026-10-02: the production SSH composition has no full shared runtime contract |
...
| `runtime.builtin.t3bridge` | production_provider | — | `runtime.Provider` | `internal/runtime/t3bridge.NewSeamBacked` | runtime.builtin/exact:t3bridge | `runtime.Provider` | waived by gascity-82e through 2026-10-02: the production T3 bridge composition has focused tests but no full shared runtime contract |
| `runtime.builtin.tmux` | production_provider | — | `runtime.Provider` | `internal/runtime/tmux.NewSeamBackedWithConfig` | runtime.builtin/exact:tmux | `runtime.Provider` | waived by gascity-82e through 2026-10-02: the existing full conformance run skips when the tmux executable is absent |

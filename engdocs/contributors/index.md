...
- [Darwin `/proc` Fallback Audit](darwin-proc-fallback-audit.md) when touching
  any procfs read — it records, per call site, whether the portable fallback is
  actually reached on macOS and whether its parser matches what `ps`/`lsof`
  print there, and names the sites that are still inert

...
func buildDoctorChecks(cityPath string, cfg *config.City, cfgErr error, opts buildDoctorChecksOpts) []doctor.Check {
	...
	if cfgErr == nil && cfg != nil {
		...
		register(newRoutedToHumanCheck(cfg, cityPath, storeFactory))
	...
	if cfgErr == nil && cfg != nil {
		...
		for _, rig := range cfg.Rigs {
			...
			register(doctor.NewRigGitHooksCheck(rig))
			register(doctor.NewSessionBranchUpstreamCheck(rig, workdirutil.WorktreesRoot(cityPath)))
